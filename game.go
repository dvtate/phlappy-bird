...
export function draw() {
  var f:int;
  if (d_a[4]:int > -100 && d_a[5]:float != 0.0f) {
    if (d_a[2]:int > -100 && d_a[3]:float != 0.0f) {
...
export function loop() {
  ...
  var o:int;
  var p:float;
  var a:int = d_a[4]:int > -100;
  var b:int = d_a[5]:float != 0.0f;
  if (a) { c = b } else { c = 0 }
  if (c) {
    var d:int = d_a[2]:int > -100;
    var e:int = d_a[3]:float != 0.0f;
    if (d) { f = e } else { f = 0 }
    if (f) {
      js_drawThwomp(50, d_a[0]:float, 40, 45);
      js_setFill(85, 255, 85);
